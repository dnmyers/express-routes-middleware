...
describe("", function () {
    ...
    it("", function () {
        const logMatcher =
            /app\.(get|post|put|delete)\([\s\S]+?logRequest\(([\s\S]+?)\)/g;
        const codeMatches = code.match(logMatcher);
        expect(
            codeMatches.length,
            "Did you use logRequest at the beginning of all the route callbacks?"
        ).to.equal(7);
        ...
        expect(
            shouldNotMatch,
            "Did you remove all the redundant `console.log` statements?"
        ).to.be.false;
    });
});
