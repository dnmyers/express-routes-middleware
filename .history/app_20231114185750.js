...
app.put("/beans/:beanName", (req, res, next) => {
    ...
    if (body.name && body.number) {
        ...
        jellybeanBag[body.name] = jellybeanBag[beanName];
        jellybeanBag[beanName] = null;
        res.send(jellybeanBag[body.name]);
    } else if (body.number) {
        jellybeanBag[beanName].number = Number(body.number);
        res.send(jellybeanBag[beanName]);
    } else {
        const err = new Error("Body must contain either a name or a number");
        err.status = 400;
        return next(err);
        // return res.status(400).send("Body must contain either a name or a number");
    }
});
